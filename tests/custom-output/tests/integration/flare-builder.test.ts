/**
 * FlareBuilder Integration Tests - Custom Output Path
 *
 * Verifies FlareBuilder query chaining works correctly with custom output path
 */
...
describe('FlareBuilder - Custom Output Path', () => {
  beforeEach(async () => {
    await cleanDatabase();
    resetCounters();
  });
  ...
  describe('DB.models Pattern', () => {
    ...
    it('should query with DB.users.where()', async () => {
      await DB.users.create({ email: 'query@test.com', name: 'Query User' });

      const users = await DB.users.where({ email: 'query@test.com' }).findMany();
    ...
    it('should chain queries with DB.users', async () => {
      ...
      const users = await DB.users.order({ name: 'asc' }).limit(2).findMany();
    ...
    it('should include relations with DB.users', async () => {
      const user = await DB.users.create({ email: 'include@test.com', name: 'Include User' });
      await DB.posts.create({ title: 'User Post', authorId: user.id });
